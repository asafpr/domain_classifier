...
def process_command_line(argv):
    ...
    parser = argparse.ArgumentParser(
        description='Filter a kraken DB file according to its domain predictions. Only filter Bacteria that looks like Eukaryota and Eukaryota that looks like Bacteria.',
...
def match_txid(domain, taxid, curr, kingdoms):
    """
...
def main(argv=None):
    ...
    with open(settings.table, 'rb') as tbin:
        for row in csv.DictReader(tbin, delimiter="\t"):
            ...
            if (float(row['Bacteria'])-float(row['Eukaryota'])>settings.mindiff):
                accdom[row['Record']] = domains['Bacteria']
            elif (float(row['Eukaryota'])-float(row['Bacteria'])>settings.mindiff):
                accdom[row['Record']] = domains['Eukaryota']
    ...
    for record in SeqIO.parse(settings.fasta, 'fasta'):
        ...
        if txid and acc in accdom:
            if not match_txid(accdom[acc], txid, curr, (domains['Eukaryota'], domains['Bacteria'])): 
                continue
        if not txid:
